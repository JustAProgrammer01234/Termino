...
class Fun(commands.Cog):
    ...
    async def randomnum(self, ctx, start: int, stop: int):
        ...
        description = random.randint(start, stop), color = discord.Colour(0xFFFF))
    ...
    async def _8ball(self, ctx, *, question: converters.eightball):
        ...
        '''
        embd = discord.Embed(title = '8ball says:', description = question[0], color = discord.Colour(0xFFFF))
    ...
    async def slap(self, ctx, member: discord.Member, *, reason):
        ...
        '''
        embd = discord.Embed(description = f'{ctx.author.mention} slapped {member.mention} because of **{reason}**', color = discord.Colour(0xFFFF))
    ...
    async def randomslap(self, ctx, *, reason: converters.SlapSomeone):
        ...
        '''
        embd = discord.Embed(description = reason, color = discord.Colour(0xFFFF))
        embd.set_image(url = random.choice(self.gif_url))
        await ctx.send(embed = embd)
    ...
    async def hack(self, ctx, member: commands.MemberConverter):
        ...
        hack_embed_message = discord.Embed(title = f'Credentials of {member.name}#{member.discriminator}', color = discord.Colour(0xFFFF))
